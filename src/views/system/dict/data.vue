<template>
   <div class="app-container">
      <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
         <el-form-item label="字典名称" prop="queryParams.parentId">
            <el-select v-model="queryParams.parentId" style="width: 200px">
               <el-option
      ...
      <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
         ...
         <el-table-column label="字典标签" align="center" prop="name">
            <template #default="scope">
               <span v-if="(scope.row.listClass == '' || scope.row.listClass == 'default') && (scope.row.cssClass == '' || scope.row.cssClass == null)">{{ scope.row.name }}</span>
               <el-tag v-else :type="scope.row.listClass == 'primary' ? '' : scope.row.listClass" :class="scope.row.cssClass">{{ scope.row.name }}</el-tag>
            </template>
         </el-table-column>
...
function getTypes(dictId) {
  getType(dictId).then(response => {
    queryParams.value.parentId = response.data.id;
    queryParams.value.parentName = response.data.name;
    defaultDictType.value = response.data.id;
    defaultDictTypeName.value = response.data.name;
    getList();
  });
}
...
function getTypeList() {
   getDictOptionselect({ level: 1 }).then(response => {
      typeOptions.value = response.data.records;
   });
}
...
function resetQuery() {
  ...
  queryParams.value.parentId = defaultDictType.value;
  handleQuery();
}
...
function handleAdd() {
  ...
  form.value.parentName = queryParams.value.parentName;
}
    ...
    if (valid) {
      if (form.value.id != undefined) {
        updateData(form.value).then(response => {
          useDictStore().removeDict(queryParams.value.parentId);
